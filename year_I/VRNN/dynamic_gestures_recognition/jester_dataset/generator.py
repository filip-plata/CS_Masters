    ...
    with open(csv_name, "rt") as csv_file:
      ...
      for row in reader:
        if len(row) < 2:
            continue
        labels.append(row[1])
        filepaths.append("20bn-jester-v1/" + str(row[0]))
    ids = list(range(len(labels)))
    labels = LabelBinarizer().fit_transform(labels)
    return DataGenerator(ids, labels, filepaths)

train_generator = createDataGenerator("jester-v1-train.csv")
test_generator = createDataGenerator("jester-v1-validation.csv")
